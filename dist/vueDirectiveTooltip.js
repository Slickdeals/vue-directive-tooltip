...
/******/ 	return __webpack_require__(__webpack_require__.s = "80d7");
/******/ })
/************************************************************************/
/******/ ({
...
// CONCATENATED MODULE: /Users/ruben.medina/.nvm/versions/node/v16.15.0/lib/node_modules/@vue/cli-service-global/node_modules/@vue/cli-service/lib/commands/build/setPublicPath.js
...
var src_css = __webpack_require__("e9cc");
...
class tooltip_Tooltip {
  constructor(el, options = {}) {
    ...
    this._options = { ...tooltip_Tooltip._defaults,
      ...{
        onFirstUpdate: data => {
          this.content(this.tooltip.options.title); // this._$tt.update();
        }
      },
      ...tooltip_Tooltip.filterOptions(options)
    };
    this._$el = el;
    this._$tpl = this._createTooltipElement(this.options);
    this._$tt = popper_createPopper(el, this._$tpl, this._options);
    this.setupPopper();
  }
  ...
  setupPopper() {
    ...
    this._$tt.setOptions(options => ({ ...options,
    ...
    if (autoHide === true) {
      this._clearDelay = setTimeout(() => {
        ...
        if (this.visible === true && this.disabled !== true) {
          // add tooltip node
          this._$el.insertAdjacentElement('afterend', this._$tpl); // Need the timeout to be sure that the element is inserted in the DOM
          ...
          setTimeout(() => {
            ...
            this._$tt.update(); // switch CSS


            this._$tpl.classList.replace(CSS.HIDDEN, CSS.VISIBLE);
          }, 60);
        } else {
          this._$tpl.classList.replace(CSS.VISIBLE, CSS.HIDDEN); // remove tooltip node
  ...
  _createTooltipElement(options) {
    ...
    $popper.setAttribute('class', `${BASE_CLASS} ${this._options.class}`); // make arrow
  ...
  _events(type = EVENTS.ADD) {
    ...
    if (includes(this.options.triggers, 'manual')) {
      ...
      if (this.options.fixIosSafari && tooltip_Tooltip.isIosSafari() && includes(this.options.triggers, 'hover')) {
  ...
  content(content) {
    ...
    } else if (tooltip_isElement(content)) {
  ...
  static filterOptions(options) {
    ...
    if (opt.placement.indexOf('-') > -1) {
      ...
      opt.placement = includes(PLACEMENT, head) && includes(SUB_PLACEMENT, tail) ? opt.placement : tooltip_Tooltip._defaults.placement;
    } else {
      opt.placement = includes(PLACEMENT, opt.placement) ? opt.placement : tooltip_Tooltip._defaults.placement;
    }

    opt.modifiers.offset = {
      fn: tooltip_Tooltip._setOffset
    };
    return opt;
  }
  ...
  static _setOffset(data, opts) {
    ...
    if (window.isNaN(offset) || offset < 0) {
      offset = tooltip_Tooltip._defaults.offset;
    }
  ...
  static defaults(data) {
    ...
    // }
    tooltip_Tooltip._defaults = { ...tooltip_Tooltip._defaults,
      ...data
    };
  }

}
tooltip_Tooltip._defaults = { ...tooltip_DEFAULT_OPTIONS
};
...
function tooltip_isElement(value) {
...
const tooltip_directive_BASE_CLASS = 'vue-tooltip';
...
function filterBindings(binding, vnode) {
  const delay = !binding.value || isNaN(binding.value.delay) ? tooltip_Tooltip._defaults.delay : binding.value.delay;
...
function getClass({
  ...
  if (value === null) {
    ...
    return `${tooltip_directive_BASE_CLASS} ${value.class}`;
  } else if (tooltip_Tooltip._defaults.class) {
    return `${tooltip_directive_BASE_CLASS} ${tooltip_Tooltip._defaults.class}`;
  } else {
    return tooltip_directive_BASE_CLASS;
  }
}
...
function update(el, binding, vnode, oldVnode) {
  if (typeof binding.value === 'string') {
    ...
    if (binding.value && binding.value.class && binding.value.class.trim() !== el.tooltip.options.class.replace(tooltip_directive_BASE_CLASS, '').trim()) {
      el.tooltip.class = `${tooltip_directive_BASE_CLASS} ${binding.value.class.trim()}`;
    }
...
// CONCATENATED MODULE: /Users/ruben.medina/.nvm/versions/node/v16.15.0/lib/node_modules/@vue/cli-service-global/node_modules/@vue/cli-service/lib/commands/build/entry-lib.js
